...
def test_add_post_create(client, django_user_model):
    user = create_user(client, django_user_model)
    url = reverse('add_post')
    form = {"title":"test", "description":"test", "latitude":"50", "longitude":"90", "user": user}
...
def test_view_post(client, django_user_model):
    user = create_user(client, django_user_model)
    post = Post.objects.create(title="test", description="test", latitude="0", longitude="0", user=user)
